...
const EXPECTED_RELEASE_LINE = 'public_source_module_extraction_second_slice_first_slice_gate';
...

{
  ...
  assert.strictEqual(output.contract_schema, 'agent-onboard-public-release-contract-026');
...

{
  ...
  assert.strictEqual(output.version, EXPECTED_VERSION);
  assert.strictEqual(output.release_line, EXPECTED_RELEASE_LINE);
  assert.strictEqual(output.contract.schema, 'agent-onboard-public-release-contract-026');
...

{
  ...
  assert.strictEqual(output.release_line, EXPECTED_RELEASE_LINE);
  assert.strictEqual(output.contract_schema, 'agent-onboard-public-release-contract-026');
  assert.strictEqual(output.fixture_matrix.schema, 'agent-onboard-public-release-fixture-matrix-019');
...

{
  ...
  assert.strictEqual(output.map.public_source_shape.physical_domain_split_status, 'source_module_second_slice_first_slice_applied');
...

{
  ...
  assert.deepStrictEqual(output.physical_module_paths_present, ['src/domains/core.js', 'src/domains/authority.js']);
...

{
  ...
  assert.ok(output.source_work_items_ledger.open_work_items.some((item) => item.title === 'Public source module extraction authority bundle parity gate'));
...

{
  ...
  assert.strictEqual(s3m1w18.status, 'open');
...

{
  ...
  assert.strictEqual(cli.PUBLIC_RELEASE_FIXTURE_MATRIX.schema, 'agent-onboard-public-release-fixture-matrix-019');
