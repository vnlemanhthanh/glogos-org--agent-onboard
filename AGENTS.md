...
```sh
npx agent-onboard@0.0.34 target onboarding --plan
npx agent-onboard@0.0.34 target onboarding --fixture
npx agent-onboard@0.0.34 target onboarding --trial
```
...
```sh
npx agent-onboard@0.0.34 target onboarding --write
```
...
```sh
npx agent-onboard@0.0.34 architecture --map
npx agent-onboard@0.0.34 architecture --router
npx agent-onboard@0.0.34 architecture --facades
npx agent-onboard@0.0.34 architecture --check
```

Inspect the public authority first-read index before expanding authority or target onboarding behavior:

```sh
npx agent-onboard@0.0.34 authority --first-read
npx agent-onboard@0.0.34 authority --check
```

Inspect the public target runtime namespace before expanding target runtime files:

```sh
npx agent-onboard@0.0.34 target runtime --namespace
npx agent-onboard@0.0.34 target runtime --check
```

Validate the public release contract and source/package surface before any package publish handoff:

```sh
npx agent-onboard@0.0.34 release --check
```

Inspect the normalized release contract:

```sh
npx agent-onboard@0.0.34 release --contract
```

Inspect the release fixture matrix:

```sh
npx agent-onboard@0.0.34 release --fixture
npx agent-onboard@0.0.34 release --surface
npx agent-onboard@0.0.34 release --surface-check
```

Run the installed package parity smoke:

```sh
npx agent-onboard@0.0.34 release --parity-smoke
npx agent-onboard@0.0.34 release --architecture-parity-smoke
```

Run the target onboarding installed-package smoke before publish handoff:

```sh
npx agent-onboard@0.0.34 release --target-onboarding-smoke
```

Inspect the post-publish verification handoff before publishing:

```sh
npx agent-onboard@0.0.34 release --post-publish-handoff
```

Run the published package acceptance rehearsal before publish, then the version-pinned acceptance check after publish:

```sh
npx agent-onboard@0.0.34 release --published-acceptance
npx agent-onboard@0.0.34 release --real-target-trial
```
...
```sh
npx agent-onboard@0.0.34 work-items --close --dry-run --id <public-work-item-id> --actor <agent-or-human-name> --summary <summary>
```
